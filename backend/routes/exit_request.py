...
def exit_request():
    ...
    pending = ExitRequest.query.filter_by(
    ...
    if pending:
        return jsonify({"message": "You already have a pending exit request"}), 400
...
def get_request(request_id):
    ...
    req = ExitRequest.query.get(request_id)
    ...
    return jsonify({
        ...
        # =========================
        # LOCKED VIOLATION SNAPSHOT (ADDED)
        # =========================
...
def history():
    ...
    try:
        ...
        # GET REQUESTS
        # =========================
        requests = ExitRequest.query.filter_by(
            student_number=student_number,
            is_deleted=False
        ).order_by(
            ExitRequest.requested_at.desc()
        ).all()
        ...
        for r in requests:
            ...
            result.append({
                ...
                "predicted_section": (
                    r.locked_section if r.locked_section else "—"
                ),

                "sanction": (
                    r.locked_sanction if r.locked_sanction else "—"
                ),
        ...
        print("EXIT REQUEST HISTORY ERROR:", str(e))
...
# =========================
@exit_request_bp.get("/admin/requests")
def admin_requests():
    ...
    for r in requests:
        ...
        # =========================
        # LATEST ACTIVE VIOLATION (fallback only)
        # =========================
        violation = None
        ...
        result.append({
            ...
            "sanction": r.locked_sanction or (
                violation.sanction if violation else None
            ),

            "violation_date": (
                r.locked_violation_date.isoformat()
                if r.locked_violation_date
                else (
                    violation.violation_date.isoformat()
                    if violation and violation.violation_date
                    else None
                )
            )
        })
...
# NOTIFICATIONS
# =========================
@exit_request_bp.get("/notifications")
def notifications():
    ...
    for r in requests:
        ...
        if r.status == "Pending":
            message = "Your exit request is pending."
        elif r.status == "Approved":
            message = "Your exit request has been approved."
        elif r.status == "Rejected":
            message = "Your exit request has been rejected."
        else:
...
# =========================
@exit_request_bp.get("/notifications/unread-count")
def unread_count():
...
# MARK AS READ
# =========================
@exit_request_bp.patch("/notifications/read/<int:request_id>")
...
# DELETE NOTIFICATION
# =========================
@exit_request_bp.delete("/notifications/<int:request_id>")
...
# Update Request 
# =========================
@exit_request_bp.patch("/request/update/<int:request_id>")
...
# =========================
@exit_request_bp.route("/request/cancel/<int:request_id>", methods=["DELETE"])
...
# =========================
@exit_request_bp.get("/admin/request/<int:request_id>")
